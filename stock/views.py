...
def stock_detail_sell(request, pk):
    ...
    return render(request, 'stockSell.html', context)
...
def stock_buy(request, pk):
    ...
    if form.is_valid():
        ...
        if acc_currency.amount < buy_cost:
            form.add_error(None, f'The account does not have enough funds in the currency {stock.currency.sign}')
        else:
...
def stock_sell(request, pk):
    ...
    if form.is_valid():
        ...
        if acc_stock.amount < amount:
            form.add_error(None, f'There are not enough shares in the account {stock.currency.sign}')
        else:
